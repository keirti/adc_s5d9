...
NX_REC nx_record0;
static NX_CALLBACK_REC g_sf_el_nx_ethernet_callbacks =
...
{ .nx_mac_address_h = SF_EL_NX_CFG_ENET0_MAC_H, .nx_mac_address_l = SF_EL_NX_CFG_ENET0_MAC_L },
  .p_callback_rec = &g_sf_el_nx_ethernet_callbacks, .etherc_ptr = R_ETHERC0, .edmac_ptr = R_EDMAC0, };
#if SF_EL_NX_CFG_IRQ_IPL != BSP_IRQ_DISABLED
#if !defined(SSP_SUPPRESS_ISR_g_sf_el_nx_ethernet) && !defined(SSP_SUPPRESS_ISR_EDMAC0)
...
VOID (*g_sf_el_nx_ethernet)(NX_IP_DRIVER *driver_req_ptr) = nx_ether_driver_eth0;
/*******************************************************************************************************************//**
 * @brief     Initialization function that the user can choose to have called automatically during thread entry.
 *            The user can call this function at a later time if desired using the prototype below.
...
uint8_t g_packet_pool0_pool_memory[(50 * (1568 + sizeof(NX_PACKET)))];
#if defined(__ICCARM__)
...
void packet_pool_init0(void)
{
    ...
    g_packet_pool0_err = nx_packet_pool_create (&g_packet_pool0, "g_packet_pool0 Packet Pool", 1568,
                                                &g_packet_pool0_pool_memory[0], (50 * (1568 + sizeof(NX_PACKET))));
...
void ip_init0(void)
{
    ...
    g_ip0_err = nx_ip_create (&g_ip0, "g_ip0 IP Instance", IP_ADDRESS (0, 0, 0, 0), IP_ADDRESS (255, 255, 255, 0),
                              &g_packet_pool0, g_sf_el_nx_ethernet, &g_ip0_stack_memory[0], 2048, 3);
