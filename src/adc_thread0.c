...
TX_THREAD adc_thread0;
static void adc_thread0_func(ULONG thread_input);
static uint8_t adc_thread0_stack[1024] BSP_PLACE_IN_SECTION_V2(".stack.adc_thread0") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
void adc_thread0_create(void)
{
    ...
    UINT err;
    err = tx_thread_create (&adc_thread0, (CHAR *) "Adc Thread 0", adc_thread0_func, (ULONG) NULL, &adc_thread0_stack,
                            1024, 1, 1, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
        tx_startup_err_callback (&adc_thread0, 0);
    }
}
